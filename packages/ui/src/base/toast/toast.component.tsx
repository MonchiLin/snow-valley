...
const ToastRender = ({ toast }: { toast: ToastOptions }): JSX.Element => {
  return typeof toast.message === 'string' ? <Text>{toast.message}</Text> : (toast.message as JSX.Element);
};
...
const ToastLevelInfo = ({ toast }: { toast: ToastOptions }) => {
  return createToastType(<AntDesign name="exclamationcircle" size={14} color="#1677ff" style={[styles.toastIcon]} />)({
    toast,
  });
};

const ToastLevelSuccess = ({ toast }: { toast: ToastOptions }) => {
  return createToastType(<AntDesign name="checkcircle" size={14} color="#52c41a" style={[styles.toastIcon]} />)({
    toast,
  });
};

const ToastLevelError = ({ toast }: { toast: ToastOptions }) => {
  return createToastType(<AntDesign name="closecircle" size={14} color="#f5222d" style={[styles.toastIcon]} />)({
    toast,
  });
};

const ToastLevelWarning = ({ toast }: { toast: ToastOptions }) => {
  return createToastType(<AntDesign name="warning" size={14} color="#faad14" style={[styles.toastIcon]} />)({ toast });
};
...
export const ToastGroupComponent = (props: { toasts: ToastOptions[]; placement: ToastOptions['placement'] }) => {
  ...
  return (
    <Animated.View
      ...
      style={[
        ...
        props.placement === 'top' ? { marginTop: safeAreaInsets.top } : { marginBottom: safeAreaInsets.bottom },
      ]}
    >
      {props.toasts.map((toast) => {
        return (
          <Animated.View
            ...
            key={toast.uniqueId}
            style={[props.placement === 'top' ? styles.toastBottomMargin : styles.toastBottomMargin]}
          >
